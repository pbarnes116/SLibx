...
Ref<MapEarthRenderer::_GISLineTile> MapEarthRenderer::_loadGISLineTile(const MapTileLocationi& location)
{
	...
	if (loader.isNotNull()) {
		Map<sl_int32, Ref<Map_GIS_Shape>> shapes = m_gisLineTileLoader.loadTile(loader, SLIB_MAP_GIS_LINE_TILE_TYPE, location);
		...
		if (tile.isNotNull()) {
			...
			{
				ListLocker< Ref<Map_GIS_Shape> > ls(shapes.values());
		...
		if (tile.isNotNull()) {
			...
			tile->flagLoaded = sl_true;
			{
				...
				for (sl_size i = 0; i < list.count(); i++) {
					...
					p.type = list[i].type;
					if (list[i].showMinLevel <= location.level) {
						...
						if (image.isNotNull()) {
							font->drawString(image, 0, size.height, text, list[i].clr);
