...
void MapEarthRenderer::_renderTile(RenderEngine* engine, _Tile* tile)
{
	...
	for (layer = 0; layer < SLIB_SMAP_MAX_LAYERS_COUNT; layer++) {
		if (m_flagShowLayer[layer] && tile->layers[layer].isNotNull()) {
			m_programSurfaceTile->setTexture(1 + layer, tile->layers[layer]->texture);
			m_programSurfaceTile->setTextureAlpha(1 + layer, 1);
		} else {
			m_programSurfaceTile->setTexture(1 + layer, Ref<Texture>::null());
			m_programSurfaceTile->setTextureAlpha(1 + layer, 0);
		}
	}
	...
	if (cache.isNotNull()) {
		if (cache->picture == tile->picture && cache->dem == tile->dem) {
			...
			for (layer = 0; layer < SLIB_SMAP_MAX_LAYERS_COUNT; layer++) {
				if (cache->layers[layer] != tile->layers[layer]) {
	...
	if (flagUseCache) {
		...
		if (tile->dem.isNotNull()) {
			tile->dem->dem.makeMeshFromSphericalGlobe(MapEarth::getGlobe(), tile->positionCenter, *primitive, 17, tile->region, tile->rectangleDEM, tile->rectanglePicture, tile->rectangleLayers);
		} else {
			DEM dem;
			dem.makeMeshFromSphericalGlobe(MapEarth::getGlobe(), tile->positionCenter, *primitive, 17, tile->region, Rectangle(0, 0, 1, 1), tile->rectanglePicture, tile->rectangleLayers);
		}
		cache = new MapRenderTile;
		cache->location = tile->location;
		cache->picture = tile->picture;
		for (layer = 0; layer < SLIB_SMAP_MAX_LAYERS_COUNT; layer++) {
			cache->layers[layer] = tile->layers[layer];
		}
		cache->dem = tile->dem;
...
{
	sl_real screenRatio = 1;//(sl_real)m_viewportWidth / 1280;
...
Ref<MapEarthRenderer::_Tile> MapEarthRenderer::_getTile(const MapTileLocationi& location)
{
	...
	if (tile) {
		...
		for (sl_uint32 layer = 0; layer < SLIB_SMAP_MAX_LAYERS_COUNT; layer++) {
			...
			Ref<MapLayerTile> tileLayer = m_tilesLayer->getTileHierarchically(layer, location, &rectangleLayer);
			tile->layers[layer] = tileLayer;
			tile->rectangleLayers[layer] = rectangleLayer;
		}
				...
				for (sl_size k = 0; k < kn; k++) {
					if (it < maxt) {
						if (!flagRequestTexture) {
							...
							Ref<Texture> textureDetail;
							if (!(m_tilesBuilding->getDetailedTexture(key, (sl_uint32)k, textureDetail))) {
