#include "../../../inc/slibx/p2p/hns_client.h"
	...
	public:
		...
		void _send_L1_initializeSession()
		{
			...
			// Body
			{
				...
				writerBody.writeStringSection(getClientId(), SLIB_P2P_HOSTID_MAX_SIZE);
				writerBody.writeStringSection(getClientCertificate(), SLIB_P2P_CERTIFICATE_MAX_SIZE);
				AES aes;
				aes.setKey_SHA256(m_serviceSecret);
			...
			if (socket.isNotNull()) {
				sl_int32 m = socket->sendTo(getServiceAddress(), bufPacket, n);
				if (m < 0) {
		...
		void _receive_L1_initializeSessionResponse(SocketAddress address, char* data, sl_size size)
		{
			...
			// Body
			{
				...
				AES aes;
				aes.setKey_SHA256(m_sessionSecret);
				char bufBody[PACKET_SIZE];
				...
				if (!readerBody.readStringSection(&strSocketAddress, SLIB_P2P_ADDRESS_MAX_SIZE)) {
					return;
				}
		...
		sl_bool _send_L1_sendDataFromClientToService(const void* data, sl_uint32 sizeData)
		{
			...
			Ref<Socket> socket = getSocket();
			if (socket.isNotNull()) {
				sl_int32 m = getSocket()->sendTo(getServiceAddress(), bufPacket, n);
				if (m < 0) {
					return sl_false;
				}
				return sl_true;
		...
		void _receive_L2_keepAliveResponse(const void* data, sl_uint32 sizeData)
		{
			...
			if (!reader.readStringSection(&strPublicAddress, SLIB_P2P_ADDRESS_MAX_SIZE)) {
				return;
			}
		...
		void _send_L2_queryHost(String hostId)
		{
			...
			writer.writeInt8(3);
			// hostId
			writer.writeStringSection(hostId, SLIB_P2P_HOSTID_MAX_SIZE);
		...
		void _receive_L2_queryHostResponse(const void* data, sl_uint32 sizeData)
		{
			...
			HnsHostInfo info;
			if (!reader.readStringSection(&(info.clientId), SLIB_P2P_HOSTID_MAX_SIZE)) {
				return;
			}
			String addr;
			if (!reader.readStringSection(&addr, SLIB_P2P_ADDRESS_MAX_SIZE)) {
				return;
			}
			if (addr.isNotEmpty()) {
		...
		void _receive_L2_sendMessageToClient(const void* data, sl_uint32 sizeData)
		{
			MemoryReader reader(data, sizeData);
			String senderId;
			if (!reader.readStringSection(&senderId, SLIB_P2P_HOSTID_MAX_SIZE)) {
				return;
			}
			sl_uint64 messageId;
